...
import co.edu.eam.disenosoft.universidad.vista.controladores.ControladorIngreCalificaciones;
...
public class VentanaIngresarCalificaciones extends javax.swing.JInternalFrame {
	...
	private ControladorIngreCalificaciones contro;

	/**
	 * Creates new form VentanaIngresarCalificaciones
	 */
	public VentanaIngresarCalificaciones() {
		contro = new ControladorIngreCalificaciones();
		initComponents();
		setVisible(true);
	...
	private void btnBuscarDocenteActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnBuscarDocenteActionPerformed
		...
		try {
			...
			Profesor p = contro.buscarDocente(ced);
		...
		try {
			...
			if (comboCurso.getSelectedIndex() != 0) {
				if (fila >= 0) {
					...
					double valor = Double.parseDouble(JOptionPane.showInputDialog(null, "Ingrese la calificaci�n"));
					Curso c = (Curso) comboCurso.getSelectedItem();
					List<Estudiante> estudiantes = contro.estudiantes(c);
					Estudiante e = estudiantes.get(fila);
					RegistroCurso r = contro.registroEstudiante(e, c);
					Evaluacion eval = (Evaluacion) comboEvaluacion.getSelectedItem();
					Nota n = new Nota(r, eval, valor, false);
					contro.agregarNota(n, columna);
	...
	private void refrescarComboDocente(Profesor d) {
		try {
			List<Curso> lista = contro.cursosDocente(d);
			comboCurso.removeAllItems();
			comboCurso.addItem("Seleccione opci�n");
	...
	private void refrescarEvaluaciones(Curso c) {
		try {
			...
			JOptionPane.showMessageDialog(null, "no tiene evaluaciones ");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void refrescarTabla(Curso c, Evaluacion a) {
		try {
			...
			for (Estudiante lista : estudiantes) {
				String calificacion = contro.calificacionEstudiante(lista, a);
