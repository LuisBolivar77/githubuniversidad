...
/**
 * Clase encargada de implemetar los m�todos de la interface Curso
 * @author LuchoBolivar
 *
 */
public class DAOCursoJPA implements IDAOCurso{
	...
	public List<Curso> cursosAsignatura(Asignatura as) throws Exception {
		...
		Query q = en.createNamedQuery(Curso.CURSOS_ASIGNATURA);
		q.setParameter(1, as);
	...
	public List<Curso> cursosDocente (Profesor d) throws Exception{
		EntityManager em = AdministradorEntityManager.getEntityManager();
		Query q = em.createNamedQuery(Curso.LISTA_CURSOS_DOCENTE);
		q.setParameter(1, d);
